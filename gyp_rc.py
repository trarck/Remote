...
if __name__ == '__main__':
  ...
  if sys.platform == 'win32':
    args.append(os.path.join(project_root, 'rc.gyp'))
    common_fn  = os.path.join(project_root, 'common.gypi')
    options_fn = os.path.join(project_root, 'options.gypi')
    # we force vs 2010 over 2008 which would otherwise be the default for gyp
    if not os.environ.get('GYP_MSVS_VERSION'):
      os.environ['GYP_MSVS_VERSION'] = '2015'
  else:
    args.append(os.path.join(os.path.abspath(project_root), 'rc.gyp'))
    common_fn  = os.path.join(os.path.abspath(project_root), 'common.gypi')
    options_fn = os.path.join(os.path.abspath(project_root), 'options.gypi')
